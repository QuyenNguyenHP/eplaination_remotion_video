...
const fullNarrationFor = (scenes) => scenes.map((scene,index)=>`[SCENE ${index+1}]\n${clean(scene.narration)}`).join('\n\n');
...
const normalize = (body) => {
  ...
  const logoScale=Math.max(.25,Math.min(1.5,Number(body.visualSettings?.logoScale)||1));
  return {...body,topic:clean(body.topic),audience:clean(body.audience),language:clean(body.language),style:clean(body.style),promptTemplate:clean(body.promptTemplate),scenes,fullNarration:fullNarrationFor(scenes),voiceCode:clean(body.voiceCode)||voices[0][0],speedRate:Math.max(.7,Math.min(1.4,Number(body.speedRate)||1)),backgroundMusic:music.includes(body.backgroundMusic)?body.backgroundMusic:'',audioSettings:{musicWithVoice:Math.max(0,Math.min(1,Number(body.audioSettings?.musicWithVoice??.12))),voice:Math.max(0,Math.min(1.5,Number(body.audioSettings?.voice??1)))},visualSettings:{logo:selectedLogo===''?'':logos.includes(selectedLogo)?selectedLogo:'logo/logo.png',logoScale}};
};
...
app.post('/api/content/generate',async(req,res,next)=>{try{
  ...
  const basePrompt=`Viết nội dung tiếng Việt cho video giải thích khái niệm "${topic}" có thời lượng render mục tiêu khoảng 60 giây. Video gồm đúng 8 scene theo thứ tự: Hook, Định nghĩa, Mục đích, Cách hoạt động, Ví dụ, Ứng dụng thực tế, Các biến thể, Câu kết. Tổng narration của 8 scene bắt buộc từ 215 đến 235 từ tiếng Việt. Scene 1-7 nên dài 27-32 từ; scene 8 dài 12-18 từ. Mỗi narration tự nhiên, dễ đọc thành voice-over, chỉ truyền đạt một ý và nối tiếp logic với scene trước. Không mở đầu hai scene bằng cùng cấu trúc, không lặp lại tên chủ đề hoặc thông tin quá nhiều. Nếu chủ đề hoặc thuật ngữ kỹ thuật là từ viết tắt, narration của scene Định nghĩa bắt buộc nêu tên đầy đủ chính xác bằng tiếng Anh ngay sau từ viết tắt ở lần xuất hiện đầu tiên, rồi mới giải thích bằng tiếng Việt. Ví dụ: "NVMe (Non-Volatile Memory Express) là...". Giữ nguyên tên đầy đủ tiếng Anh, không dịch thay thế và không tự suy đoán phần mở rộng cho thuật ngữ không phải từ viết tắt. Quy tắc này cũng áp dụng cho mọi từ viết tắt kỹ thuật khác xuất hiện trong nội dung: giải nghĩa ở lần xuất hiện đầu tiên. Scene 5 phải có ví dụ hay dữ liệu cụ thể. Scene 8 ngắn và dễ nhớ. title của mỗi scene bắt buộc là một cụm nội dung chính bằng tiếng Việt gồm 2-4 từ. title phải tóm tắt đúng ý riêng của scene, ví dụ "Ngắt điện an toàn", "Bảo vệ quá tải", "Dòng điện ổn định"; tuyệt đối không dùng tên cấu trúc như "Hook", "Định nghĩa", "Mục đích", "Cách hoạt động", "Ví dụ", "Ứng dụng", "Biến thể" hoặc "Câu kết" làm title. visualDescription mô tả rõ hình minh họa, không mô tả chữ trong ảnh. Chỉ trả dữ liệu đúng schema JSON.`;
  ...
  res.json({topic:clean(generated.topic)||topic,scenes,contentStats:{totalWords,estimatedDuration},fullNarration:fullNarrationFor(scenes)});
}catch(e){next(e)}});
